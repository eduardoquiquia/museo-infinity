<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    @forelse ($salas as $sala)
        <div class="bg-neutral-900 rounded-xl shadow-md overflow-hidden border border-neutral-800">
            ...
            <div class="p-4 text-neutral-200">
                ...
                <h3 class="text-lg font-semibold mb-1">{{ $sala->titulo }}</h3>

                @if($sala->subtitulo)
                    <p class="text-neutral-400 text-sm mb-2 line-clamp-1">{{ $sala->subtitulo }}</p>
                @endif
                ...
                <div class="text-neutral-400 text-sm space-y-1 mb-4">
                    <div>
                        ...
                        Nivel: {{ ucfirst($sala->nivel_experiencia) }}
                    </div>

                    <div>
                        <i class="fas fa-door-closed mr-2"></i>
                        Salas incluidas: {{ $sala->salas_incluidas }}
                    </div>
                    ...
                    <div>
                        ...
                        <span
                            class="font-semibold 
                                @if($sala->estado === 'activo') text-green-400 
                                @elseif($sala->estado === 'inactivo') text-yellow-400 @endif">
                            {{ ucfirst($sala->estado) }}
                        </span>
                    </div>
                </div>
    ...
    @empty
        <p class="text-neutral-400 text-center col-span-full">No hay salas registradas.</p>
    @endforelse
</div>
